	...
	<div>
		...
		>
			<section class="ml-5 mt-5 py-3" slot="pdf-content">
				...
				<div class="mt-5">
					<p class="text-justify" style="text-indent: 40%">
						Quien suscribe {{ SocioBajaMora.nombre }}
						{{ SocioBajaMora.apellido }}, DNI {{ SocioBajaMora.dni }} Nº de
						afiliado {{ SocioBajaMora.numero_socio }} al servicio brindado por
						Ricardo Mora, quien por medio de la presente vengo a solicitar en
						forma unilateral la cancelación del servicio de salud y sepelio.
					</p>
					<p>
						Lo cual solicito que el descuento que se hace a través de mi cobro
						quede cancelado.
					</p>
					<p>
						Autorizo a {{ nota.nombre }}, con DNI {{ nota.dni }} a PRESENTAR
						esta nota.
					</p>
				</div>
		...
		<b-form-group
			label-cols="auto"
			label="Nombre que Autoriza"
			label-for="nombre_nota"
		>
			<b-form-input
				id="nombre_nota"
				v-model="nota.nombre"
...
<script>
	...
	export default {
		...
		beforeMount() {
			this.nota.fecha = new Date().toLocaleDateString("es-ar", {
				month: "long",
				day: "numeric",
				year: "numeric",
			});
