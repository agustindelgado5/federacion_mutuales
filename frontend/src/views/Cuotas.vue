							...
							<b-list-group horizontal>
								<b-list-group>
									...
									<b-list-group-item
										><b>Fecha:</b>
										{{
											row.item.fecharealizacion.split("T")[0]
										}}</b-list-group-item
									>
								</b-list-group>
								&nbsp;
									...
									>
										<b-card-body>
											...
											>
												...
												<v-date-picker v-model="dateDesde" scrollable>
													<v-spacer></v-spacer>
													...
													<v-btn
														text
														color="primary"
														@click="$refs.dialog.save(dateDesde)"
													>
														OK
													</v-btn>
												</v-date-picker>
											...
											>
												...
												<v-date-picker v-model="dateHasta" scrollable>
													<v-spacer></v-spacer>
													<v-btn text color="primary" @click="modal = false">
														Cancel
													</v-btn>
													<v-btn
														text
														color="primary"
														@click="$refs.dialog.save(dateHasta)"
													>
														OK
													</v-btn>
												</v-date-picker>
	...
	export default {
		...
		data() {
			return {
				...
				modal: false,
				/*
				dateDesde: new Date(Date.now() - new Date().getTimezoneOffset() * 60000)
					.toISOString()
					.substr(0, 10),
        ...
        */
		...
		computed: {
			...
			/*
			FilterPorFecha(){
				console.log("Filter: ",this.$options.filters.fecha_pagos_range(this.fields, this.dateDesde, this.dateHasta))
		...
		created: function(){
			console.log("Filter: ",this.$options.filters.fecha_pagos_range(this.tabla_cuot, '2021-05-01', '2021-11-31'));
		},
