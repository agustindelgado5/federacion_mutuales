	...
	<div>
		...
		<b-form>
			...
			<b-form-group label="*Cristal Derecho" label-for="cristal_derecho">
				...
				<b-form-invalid-feedback id="cristal-live-feedback"
	...
	export default {
		...
		data() {
			return {
				...
				op_lentes: [{ value: null, text: "Elija una lente", disabled: true }],
				op_cristales: [
		...
		methods: {
			getForeingKeys() {
				this.ventaOptica.numero_socio =
					"http://localhost:8081/socios/" +
					this.ventaOptica.socio.split("-")[0] +
					"/";
			},
			...
			async getLentes() {
				...
				this.data.forEach((element) => {
					let option = {};
					option.value =
						"http://localhost:8081/lentes/" + element.id_lente + "/";
					option.text =
						element.marca +
						"-- " +
						element.material +
			...
			async getCristales() {
				...
				this.data.forEach((element) => {
					let option = {};
					option.value =
						"http://localhost:8081/cristales/" + element.id_cristal + "/";
					option.text =
						element.material +
						"-- esf: " +
						element.esfera +
