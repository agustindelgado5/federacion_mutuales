    ...
    >
      <section slot="pdf-content">
        ...
        <section class="pdf-item">
          <b-card-group deck>
            <b-card img-src="../assets/logo.jpg" img-alt="Logo Federación" img-left class="mb-3" img-width="90" img-height="90">
              <b-card-text>
                <h6>Federación Tucumana de Mutuales</h6>
                <div>
                  <b-list-group horizontal="md" style="font-size: 70%">
                    <b-list-group-item>
                      <b>N°: {{carnetAPDF.numero_socio}}</b> | <b>Afiliado:</b> {{carnetAPDF.afiliado}} | <b>DNI:</b> {{carnetAPDF.dni}}
...
export default {
  components: { SociosAlta,SociosUpdate,VueAwesomplete, VueHtml2pdf },
  data() {
    return {
      tabla_socios: [],
      fields: [
  ...
  methods: {
    ...
    async generarCarnet(item) {
      ...
      for(var i=0; i<this.data.length; i++){
         if(this.data[i].numero_socio.split('/')[4]==data_socio.numero_socio){
            let data_fam={}
            data_fam.dni=this.data[i].dni_familiar
            data_fam.adherente=this.data[i].apellido +', ' + this.data[i].nombre 
            data_socio.adhrentes.push(data_fam)
        } 
      }
      
      //this.carnetAPDF = {...item};
