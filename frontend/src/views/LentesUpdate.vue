<template>
  <div>
    ...
    <b-form>
     ...
     <b-form-group label="*Codigo SEguimiento" label-for="cod_seguimiento">
        <b-form-select
            id="cod_seguimiento"
            v-model="medicamentos.cod_seguimiento"
            type="text"
            placeholder="Ingrese un Numero"
            invalid-feedback="Complete este campo"
            required
            :state="validacion.cod_seguimiento.estado"
            :options="options"
        >
        </b-form-select>
        <b-form-invalid-feedback
          id="codigo_seguimiento-live-feedback"
        >
    ...
    <b-button class="mt-2" variant="success" block @click="postLente()"
      >Guardar</b-button
    >
  </div>
</template>
...
export default {
  data() {
    return {
      ...
      options: [
        {value: null, text: 'Elija una venta', disabled: true},
      ],
  ...
  methods: {
    ...
    async getVentasOpticas() {
      ...
      this.data.forEach(element => {   
        let option={}
        option.value='http://localhost:8081/ventasOpticas/'+ element.codigo_seguimiento +'/';
